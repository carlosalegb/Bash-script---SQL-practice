...
do
  ...
  then 
    if [[ -z $TEAM ]]
    then
      INSERT_TEAM=$($PSQL "INSERT INTO teams(name) VALUES('$WINNER') RETURNING *")
      if [[ $(echo "$INSERT_TEAM" | wc -l) -eq 2 ]]
      then
  ...
  TEAMB=$($PSQL "SELECT name FROM teams WHERE name='$OPPONENT'")
  if [[ $OPPONENT != "opponent" ]]
  then 
    if [[ -z $TEAMB ]]
    then
      INSERT_TEAMB=$($PSQL "INSERT INTO teams(name) VALUES('$OPPONENT') RETURNING *")
      if [[ $(echo "$INSERT_TEAMB" | wc -l) -eq 2 ]]
      then
